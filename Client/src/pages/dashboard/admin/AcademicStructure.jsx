...
} from "../../../components/AddBtnModals.jsx";
    ...
    allocations: [
      {
        ...
        subjectCode: "CS301",
        batchName: "Batch 2021-2025 (Sec A)",
      },
      {
        id: "a2",
        subjectName: "Advanced Databases",
        subjectCode: "CS505",
        batchName: "Batch 2020-2024 (Sec B)",
      },
    ],
  },
  {
...
const deptDropdownData = [
  {
    name: "Department",
    options: ["All Departments", ...mockDepartments.map((dept) => dept.code)],
  },
];
const subjectTypeDropdownData = [
  { name: "Type", options: ["All Types", "Core", "Elective"] },
];
const semDropdownData = [
  {
    name: "Semester",
    options: [
      "All Semesters",
...
const SubjectTypeBadge = ({ type }) => {
  const theme =
    type === "Core" ? "primary" : type === "Elective" ? "info" : "warning";
  return (
    <span
      className={`badge bg-${theme} bg-opacity-10 text-${theme} border border-${theme} border-opacity-25 px-2 py-1 rounded-2`}
    >
      {type}
    </span>
  );
...
const Toolbar = ({
  ...
  <div
    ...
    style={{ color: "var(--text-main)", backgroundColor: "var(--light-hover)" }}
  >
    <div
      className="d-flex flex-wrap flex-md-nowrap gap-2 w-100"
      style={{ maxWidth: dropdowns ? "800px" : "400px" }}
    >
      ...
      {dropdowns &&
        dropdowns.map((props, idx) => (
          <div
            key={idx}
            style={{ minWidth: "220px", flexShrink: 0, zIndex: 10 }}
          >
    ...
    <button
      className={`btn rounded-3 px-3 py-2 fw-medium d-flex align-items-center gap-1 flex-shrink-0 ${theme === "light" ? "btn-outline-dark" : "btn-outline-light"}`}
  ...
  return (
    <>
      <Toolbar
        ...
        primaryActionText="Add Department"
        onPrimaryAction={() => setIsAddDeptOpen(true)}
        theme={theme}
      />
      <div className="table-responsive">
        ...
        >
          <thead style={{ fontSize: "0.875rem" }}>
            <tr>
              <th
                className="text-uppercase ps-4 py-3 border-bottom-0"
                style={{ color: "var(--text-muted)" }}
              >
                Department Name
              </th>
              <th
                className="text-uppercase py-3 border-bottom-0"
                style={{ color: "var(--text-muted)" }}
              >
                Code
              </th>
              ...
              <th
                className="text-uppercase text-end pe-4 py-3 border-bottom-0"
                style={{ color: "var(--text-muted)" }}
              >
                Actions
              </th>
          ...
          <tbody>
            {filtered.map((dept) => (
              <tr key={dept.id}>
                ...
                <td>
                  <span className="badge text-secondary bg-secondary bg-opacity-10 border border-secondary border-opacity-25 px-2 py-1 rounded">
                    {dept.code}
                  </span>
                </td>
                <td
  ...
  return (
    <>
      <Toolbar
        ...
        primaryActionText="Add Batch"
        dropdowns={[
          {
            dropdownData: deptDropdownData,
            selectedValue: deptFilter || "All Departments",
            onSelect: (_, v) => setDeptFilter(v === "All Departments" ? "" : v),
          },
        ]}
      ...
      <div className="table-responsive">
        ...
        >
          <thead style={{ fontSize: "0.875rem", color: "var(--text-muted)" }}>
            <tr>
              <th
                className="text-uppercase ps-4 py-3 border-bottom-0"
                style={{ color: "var(--text-muted)" }}
                ...
                <td>
                  <div className="d-flex flex-wrap gap-1">
                    {batch.sections.map((sec) => (
                      <span
                        key={sec}
                        className="badge text-secondary bg-secondary bg-opacity-10 border border-secondary border-opacity-25 px-2 py-1 rounded-2"
                      >
                        {sec}
                      </span>
                    ))}
    ...
    <>
      <Toolbar
        ...
        dropdowns={[
          ...
          {
            dropdownData: subjectTypeDropdownData,
            selectedValue: typeFilter || "All Types",
            onSelect: (_, v) => setTypeFilter(v === "All Types" ? "" : v),
          },
        ]}
        ...
        >
          <thead style={{ fontSize: "0.875rem" }}>
            <tr>
              ...
              <th className="text-uppercase py-3 border-bottom-0" style={{}}>
                Status
              </th>
              <th
                className="text-uppercase text-end pe-4 py-3 border-bottom-0"
          ...
          <tbody>
            {filtered.map((sub) => (
              <tr key={sub.id}>
                <td className="ps-4 fw-bold">{sub.name}</td>
                <td
                  className="fw-medium"
                  style={{
                    color: "var(--text-muted)",
...
const AllocationsTab = ({
  isAllocateFacultyOpen,
  setIsAllocateFacultyOpen,
}) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [deptFilter, setDeptFilter] = useState("");
  ...
  return (
    <>
      <Toolbar
        ...
        primaryActionText="Allocate Faculty"
        dropdowns={[
          {
            dropdownData: deptDropdownData,
            selectedValue: deptFilter || "All Departments",
        ...
        >
          <thead style={{ fontSize: "0.875rem" }}>
            <tr>
              ...
              <th
                className="text-uppercase py-3 border-bottom-0 text-center"
                style={{ color: "var(--text-muted)" }}
              >
                Assignments
              </th>
            ...
            {filtered.map((faculty) => {
              ...
              return (
                <React.Fragment key={faculty.id}>
                  <tr
                    onClick={() =>
                      setExpandedRowId(isExpanded ? null : faculty.id)
                    }
                    style={{
                      cursor: "pointer",
                      backgroundColor: isExpanded ? "var(--light-hover)" : "",
                    }}
                    // className={isExpanded ? "bg-light" : ""}
                  >
                    <td
                      className="ps-4 text-secondary"
                      style={{
                        width: "40px",
                      }}
                    >
                      <div
                        className="d-flex align-items-center justify-content-center bg-white border rounded-circle shadow-sm"
                        style={{
                          width: "24px",
                          height: "24px",
                    ...
                    <td className="fw-bold">{faculty.facultyName}</td>
                    <td>
                      <span className="badge text-secondary bg-secondary bg-opacity-10 border border-secondary border-opacity-25 px-2 py-1 rounded">
                        {faculty.deptCode}
                      </span>
                    ...
                    <td className="text-center">
                      <span className="badge bg-dark rounded-pill px-2 py-1">
                  ...
                  {isExpanded && (
                    <tr>
                      <td colSpan="6" className="p-0 border-bottom-0">
                        <div
                          className="bg-secondary bg-opacity-10 px-4 py-4 border-start border-4 border-primary"
                          style={{
                        ...
                        >
                          <div className="d-flex justify-content-between align-items-center mb-3 px-1">
                            ...
                            <button
                              className={`btn btn-sm hover-lift py-1 px-3 d-flex align-items-center gap-1 ${theme === "dark" ? "btn-outline-light" : "btn-outline-dark"}`}
                            >
                              {ICONS.plus} Quick Allocate
                            </button>
                          </div>
                          {faculty.allocations.length > 0 ? (
                            ...
                            >
                              <p
                                className="fw-medium small mb-0"
                                style={{ color: "var(--text-muted)" }}
                              >
                                No subjects allocated to this faculty member
                                yet.
                              </p>
                            </div>
                          )}
...
const AcademicStructure = () => {
  ...
  const tabContent = {
    ...
    allocations: (
      <AllocationsTab
        isAllocateFacultyOpen={isAllocateFacultyOpen}
        setIsAllocateFacultyOpen={setIsAllocateFacultyOpen}
      />
    ),
  };
  ...
  return (
    <div
      className="academic-structure-container fade-in"
    ...
    >
      <div className="text-center mt-5 mb-5">
        ...
        <p className="fw-medium fs-6" style={{ color: "var(--text-muted)" }}>
          Manage departments, curriculum, and faculty allocations.
        </p>
      </div>

      <div className="card px-5 bg-transparent border-0">
        <div>
          <ul
            className="border border-secondary-subtle gap-1 bg-secondary-subtle mx-auto rounded-pill d-flex flex-row align-items-center p-1 list-unstyled"
        ...
        <div
          className="rounded-4 overflow-hidden mt-3"
      ...
      />
      <AllocateFacultyModal
        isOpen={isAllocateFacultyOpen}
        onClose={() => setIsAllocateFacultyOpen(false)}
      />
    </div>
  );
};
